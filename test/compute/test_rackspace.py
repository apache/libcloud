...
from libcloud.compute.drivers.rackspace import RackspaceNodeDriver, RackspaceResponse
from libcloud.pricing import clear_pricing_data, set_pricing

from test import MockHttpTestCase, MockResponse, XML_HEADERS
...
class RackspaceResponseTestCase(unittest.TestCase):
    XML = """<?xml version="1.0" encoding="UTF-8"?><root/>"""

    def test_simple_xml_content_type_handling(self):
        http_response = MockResponse(200, RackspaceResponseTestCase.XML, headers={'content-type': 'application/xml'})
        body = RackspaceResponse(http_response).parse_body()
...
class RackspaceTests(unittest.TestCase, TestCaseMixin):
    ...
    def setUp(self):
        self.driver_type.connectionCls.conn_classes = (RackspaceMockHttp, RackspaceMockHttp)
    ...
    def test_list_sizes_with_specified_pricing(self):
        ...
        set_pricing(driver_type='compute', driver_name='rackspace',
                    pricing=pricing)
