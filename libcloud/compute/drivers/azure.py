...
class AzureNodeDriver(NodeDriver):
    ...
    def reboot_node(self,
        ...
        ex_cloud_service_name = ex_cloud_service_name or (node.extra and node.extra.get('ex_cloud_service_name'))
    ...
    def _get_cloud_service_location(self, service_name=None):
        ...
        res = self._perform_get(
            self._get_hosted_service_path(service_name) + '?embed-detail=False',
            HostedService
        )
    ...
    def _convert_class_to_xml(self, source, xml_prefix=True):
        ...
        if isinstance(source, list):
            xmlstr += ''.join(self._convert_class_to_xml(value, False) for value in source)
