...
class CloudStackCommonTestCase(TestCaseMixin):
    ...
    def test_create_node_ex_keyname(self):
        ...
        node = self.driver.create_node(
            name="test", location=location, image=image, size=size, ex_keyname="foobar"
        )
        self.assertEqual(node.name, "test")
    ...
    def test_create_node_ex_userdata(self):
        ...
        CloudStackMockHttp.fixture_tag = "deploykeyname"
        node = self.driver.create_node(
            name="test", location=location, image=image, size=size, ex_userdata="foobar"
        )
        self.assertEqual(node.name, "test")
    ...
    def test_create_node_project(self):
        ...
        node = self.driver.create_node(
            name="test", location=location, image=image, size=size, project=project
        )
    ...
    def test_ex_create_network_acllist(self):
        ...
        network_acllist = self.driver.ex_create_network_acllist(
            name="test_acllist", vpc_id=vpc.id, description="test description"
        )
    ...
    def test_ex_list_projects(self):
        ...
        for i, project in enumerate(projects):
            ...
            self.assertEqual(project.display_text, fixture_projects[i]["displaytext"])
            self.assertEqual(project.name, fixture_projects[i]["name"])
            self.assertEqual(project.extra["domainid"], fixture_projects[i]["domainid"])
            self.assertEqual(project.extra["cpulimit"], fixture_projects[i]["cpulimit"])
    ...
    def test_create_volume_with_defined_volume_type(self):
        ...
        volume = self.driver.create_volume(
            10, volumeName, location=volLocation, ex_volume_type=volumeType
        )
    ...
    def test_ex_authorize_security_group_ingress(self):
        res = self.driver.ex_authorize_security_group_ingress(
            "test_sg", "udp", "0.0.0.0/0", "0", "65535"
        )
    ...
    def test_ex_create_firewall_rule(self):
        ...
        rule = self.driver.ex_create_firewall_rule(
            address, cidr_list, protocol, start_port=start_port, end_port=end_port
        )
        self.assertEqual(rule.address, address)
        self.assertEqual(rule.protocol, protocol)
        self.assertIsNone(rule.icmp_code)
    ...
    def test_ex_create_firewall_rule_icmp(self):
        ...
        icmp_type = 8
        rule = self.driver.ex_create_firewall_rule(
            address, cidr_list, protocol, icmp_code=icmp_code, icmp_type=icmp_type
        )
        self.assertEqual(rule.address, address)
        self.assertEqual(rule.protocol, protocol)
        self.assertEqual(rule.icmp_code, 0)
    ...
    def test_ex_create_egress_firewall_rule(self):
        ...
        rule = self.driver.ex_create_egress_firewall_rule(
            network_id, cidr_list, protocol, start_port=start_port, end_port=end_port
        )
    ...
    def test_node_ex_create_port_forwarding_rule(self):
        ...
        openfirewall = True
        protocol = "TCP"
        rule = node.ex_create_port_forwarding_rule(
            address,
            private_port,
            public_port,
            protocol,
...
class CloudStackMockHttp(MockHttp, unittest.TestCase):
    ...
    def _test_path_api_error(self, method, url, body, headers):
        ...
        return (431, body, {}, httplib.responses[httplib.OK])
