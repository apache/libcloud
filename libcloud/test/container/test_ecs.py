...
class ElasticContainerDriverTestCase(unittest.TestCase):
    ...
    def test_deploy_container(self):
        container = self.driver.deploy_container(
            ...
            image=ContainerImage(
                id=None, name="mysql", path="mysql", version=None, driver=self.driver
            ),
        )
        self.assertEqual(
            container.id,
    ...
    def test_get_container(self):
        ...
        self.assertEqual(
            container.id,
            "arn:aws:ecs:ap-southeast-2:647433528374:container/d56d4e2c-9804-42a7-9f2a-6029cb50d4a2",
        )
    ...
    def test_stop_container(self):
        container = self.driver.stop_container(
            Container(
                ...
                ip_addresses=None,
                driver=self.driver,
                extra={"taskArn": "12345", "taskDefinitionArn": "123556"},
            )
        )
        self.assertFalse(container is None)
    ...
    def test_list_images(self):
        ...
        self.assertEqual(
            images[0].name, "647433528374.dkr.ecr.region.amazonaws.com/my-images:latest"
        )

    def test_ex_create_service(self):
        cluster = self.driver.list_clusters()[0]
        task_definition = self.driver.list_containers()[0].extra["taskDefinitionArn"]
        service = self.driver.ex_create_service(
            cluster=cluster, name="jim", task_definition=task_definition
        )
